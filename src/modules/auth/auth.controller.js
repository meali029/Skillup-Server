...
export const register = async (req, res) => {
  try {
    ...
    res.cookie("token", token, cookieOptions).status(201).json({ user: { id: user._id, name: user.name, email: user.email }});
  } catch (err) {
...
export const login = async (req, res) => {
  try {
    ...
    res.cookie("token", token, cookieOptions).json({ user: { id: user._id, name: user.name, email: user.email }});
  } catch (err) {
...
export const googleCallback = async (req, res) => {
  try {
    ...
    // Redirect to client's Google callback handler
